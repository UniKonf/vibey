...
interface getHackathonDataValue extends SingleHackathonType {
  [key: string]: any;
}
function EditHackathon() {
  ...
  // const id = '64d25e817e9d1e60534cdf92';
  ...
  //   ideas: { count: 0, array: [] },
  // });
  const [hackathonData, setHackathonData] = useState<getHackathonDataValue>({
    _id: '',
    name: '',
    slug: '',
    organizer: '',
    description: '',
    mode: { isOnline: false, location: '' },
    date: new Date(),
    deadline: new Date(),
    rewards: { title: '', prize: '' },
    size: 0,
  ...
  const getData = async () => {
    ...
    const response = await fetch(
      ...
      }/api/hackathons/id/${id}`
    ).then((res) => res.json());

    if (response.success) {
      setHackathonData(response.hackathon[0]);
      setRewardsDetails(response.hackathon[0].rewards);
      setIsOnlineHackathon(response.hackathon[0].mode);
      // setSelectedTags(response.hackathon[0].tags);
      // setSponsorsDetails(response.hackathon[0].sponsors);
      // setSpeakerDetails(response.hackathon[0].speakers);
      // setIsTicketRequires(response.hackathon[0].requiresTicket);
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (formRef.current) {
      ...
      try {
        const res = await fetch('/api/hackathons/editHackathon', {
          method: 'post',
          headers: {
  ...
  const handleRewardsCount = (e: any, type: string, index = 0) => {
    ...
    if (type === 'remove') {
      const newReward = rewardsDetails.filter(
        (_, currentIndex: number) => currentIndex !== index
      );
      setRewardsDetails(newReward);
    }
  };
  // inputs
  const inputs = [
                ...
                <div>
                  {input.element === 'textarea' && (
                    <textarea
                      ...
                      name={input.name}
                      style={{ color: 'black' }}
                      defaultValue={hackathonData.description}
                  ...
                  {input.element === 'input' && (
                    <input
                      ...
                      defaultValue={
                        hackathonData[
                          input.name as keyof SingleHackathonType
                        ] as any
                      }
            ...
            pointer-events-none inline-block h-[34px] w-[34px] transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out`}
                      ...
                      {!isOnlineHackathon.isOnline && (
                        <input
                          ...
                          id={input.name}
                          name={input.name}
                          defaultValue={hackathonData?.mode?.location as string}
