...
interface getCfpDataValue extends CfpType {
  [key: string]: any;
}
...
function AddCfp() {
  ...
  const [cfpData, setCfpData] = useState<getCfpDataValue>({
    name: '',
    slug: '',
    organizer: '',
    description: '',
  ...
  const getData = async () => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_ENDPOINT as string}/api/cfps/id/${id}`
    ).then((res) => res.json());

    if (response.success) {
      setCfpData(response.cfps[0]);
      setIsOnlineCfp(response.cfps[0].address);
      setSelectedTags(response.cfps[0].tags);
      setTopicsDetails(response.cfps[0].topics);
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (formRef.current) {
      ...
      try {
        const res = await fetch('/api/cfps/editCfp', {
          method: 'post',
          headers: {
                ...
                <div>
                  {input.element === 'textarea' && (
                    <textarea
                      ...
                      name={input.name}
                      style={{ color: 'black' }}
                      defaultValue={cfpData.description}
                      ...
                      {topicsDetails.map((topic, index: number) => (
                        <div key={index}>
                          ...
                          <Button
                            className="mt-6"
                            onClick={(e) =>
                              handleTopicsCount(e, 'remove', index)
                            }
                          >
                            Delete Topic
                          </Button>
                  ...
                  {input.element === 'input' && (
                    <input
                      ...
                      aria-label={input.placeholder}
                      aria-describedby={input.placeholder}
                      defaultValue={cfpData[input.name as keyof CfpType] as any}
                    />
                  )}
            ...
            pointer-events-none inline-block h-[34px] w-[34px] transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out`}
                      ...
                      {!isOnlineCfp.isOnline && (
                        <input
                          ...
                          id={input.name}
                          name={input.name}
                          defaultValue={cfpData?.address?.location as string}
